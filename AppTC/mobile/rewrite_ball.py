...
class CatBallThrowGame extends StatefulWidget {
  const CatBallThrowGame({Key? key}) : super(key: key);

  @override
  State<CatBallThrowGame> createState() => _CatBallThrowGameState();
}

class _CatBallThrowGameState extends State<CatBallThrowGame> with TickerProviderStateMixin {
  int score = 0;
  int combo = 0;
  int totalXp = 0;
  int totalCoin = 0;
  ...
  void initState() {
    ...
    _throwAnimation = Tween<Offset>(begin: Offset.zero, end: Offset.zero).animate(_throwController);
    _scaleAnimation = Tween<double>(begin: 1.0, end: 1.0).animate(_throwController);
    _rotateAnimation = Tween<double>(begin: 0.0, end: 0.0).animate(_throwController);
  ...
  void _onPanEnd(DragEndDetails details, double screenHeight) {
    ...
    final throwVector = Offset(-dragOffset.dx * throwMultiplier, -dragOffset.dy * throwMultiplier);

    _throwAnimation = Tween<Offset>(
      begin: currentBallOffset,
      end: throwVector,
  ...
  void _checkScore() {
    ...
    if (distanceX < 60 && distanceY < 120) {
      ...
      _showSnack('VÀO RỔ! +10 Điểm (Combo x$combo) 🔥', const Color(0xFF4CAF50));
      _resetBall();
    } else {
      setState(() {
        combo = 0;
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Row(
          ...
          children: [
            if (color == const Color(0xFF4CAF50)) const Icon(Icons.sports_basketball, color: Colors.white),
            if (color == const Color(0xFFF44336)) const Icon(Icons.cancel, color: Colors.white),
            const SizedBox(width: 10),
            Text(msg, style: GoogleFonts.nunito(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.white)),
          ],
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text('Mèo Ném Bóng', style: GoogleFonts.nunito(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 24, shadows: const [Shadow(blurRadius: 4, color: Colors.black54)])),
          centerTitle: true,
          backgroundColor: Colors.transparent,
          elevation: 0,
          leading: IconButton(
            icon: const Icon(Icons.arrow_back_ios_new, color: Colors.white, shadows: [Shadow(blurRadius: 4, color: Colors.black54)]),
        ...
        body: Container(
          ...
          decoration: const BoxDecoration(
            gradient: LinearGradient(
              colors: [Color(0xFF1E1E2C), Color(0xFF3B2F4C), Color(0xFF5A4465), Color(0xFF9E768F)],
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text('ĐIỂM: $score', style: GoogleFonts.nunito(fontSize: 22, fontWeight: FontWeight.w900, color: Colors.white)),
                      if (combo > 0)
                        Padding(
                          padding: const EdgeInsets.only(top: 4),
                          child: Text('Combo x$combo 🔥', style: GoogleFonts.nunito(fontSize: 16, color: Colors.orangeAccent, fontWeight: FontWeight.w900)),
                        ),
                      const SizedBox(height: 8),
                      Row(
                        children: [
                          const Icon(Icons.star_rounded, color: Colors.yellowAccent, size: 20),
                          const SizedBox(width: 4),
                          Text('$totalXp XP', style: GoogleFonts.nunito(fontSize: 16, color: Colors.white, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ],
                  ),
              ...
              Positioned(
                ...
                child: GestureDetector(
                  ...
                  child: Transform.scale(
                    scale: _throwController.isAnimating ? _scaleAnimation.value : 1.0,
                    child: Transform.rotate(
                      angle: _throwController.isAnimating ? _rotateAnimation.value : 0.0,
                      child: SizedBox(
                        width: 90,
                        height: 90,
                        child: CustomPaint(
...
class BackgroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    floorPath.quadraticBezierTo(size.width * 0.5, size.height * 0.8, size.width, size.height * 0.85);
...
class BackboardPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRRect(rrect, Paint()..color = Colors.grey.shade400..style = PaintingStyle.stroke..strokeWidth = 6);
    
    // Inner red square
    final innerRect = Rect.fromLTWH(size.width / 2 - 35, size.height - 60, 70, 45);
    canvas.drawRect(innerRect, Paint()..color = Colors.redAccent..style = PaintingStyle.stroke..strokeWidth = 5);
    
    // Base connector
    canvas.drawRect(Rect.fromLTWH(size.width / 2 - 15, size.height, 30, 20), Paint()..color = Colors.grey.shade800);
  }
  @override bool shouldRepaint(covariant CustomPainter oldDelegate) => false;
}
...
class RimAndNetPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    // Rim
    final rimRect = Rect.fromCenter(center: Offset(size.width / 2, 0), width: size.width, height: 25);
    canvas.drawOval(rimRect, Paint()..color = Colors.orange.shade800..style = PaintingStyle.stroke..strokeWidth = 6);
    // Rim inner shadow
    canvas.drawOval(rimRect, Paint()..color = Colors.deepOrange..style = PaintingStyle.stroke..strokeWidth = 2);
    ...
    for (int i = 0; i <= 5; i++) {
       ...
       double endX = (size.width - netWidthBottom) / 2 + (netWidthBottom / 5) * i;
    ...
    for (int i = 0; i <= 3; i++) {
        ...
        double currentWidth = netWidthTop - ((netWidthTop - netWidthBottom) / 3 * i);
...
class TrajectoryPainter extends CustomPainter {
  ...
  @override bool shouldRepaint(covariant TrajectoryPainter oldDelegate) => oldDelegate.dragOffset != dragOffset;
}
...
class CatBallPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawArc(Rect.fromCircle(center: Offset(-10, cy), radius: r * 1.1), -pi/2, pi, false, linePaint);
    canvas.drawArc(Rect.fromCircle(center: Offset(size.width + 10, cy), radius: r * 1.1), pi/2, pi, false, linePaint);

    // Cat Face On The Ball
    final catColor = Paint()..color = Colors.white.withOpacity(0.9);
    ...
    // Eyes
    canvas.drawCircle(Offset(cx - 15, cy - 10), 6, Paint()..color = Colors.black);
    canvas.drawCircle(Offset(cx + 15, cy - 10), 6, Paint()..color = Colors.black);
    canvas.drawCircle(Offset(cx - 17, cy - 12), 2, Paint()..color = Colors.white);
    canvas.drawCircle(Offset(cx + 13, cy - 12), 2, Paint()..color = Colors.white);

    // Nose
    canvas.drawCircle(Offset(cx, cy - 2), 3, Paint()..color = Colors.pinkAccent);
    ...
    canvas.drawPath(mouth, Paint()..color = Colors.black87..style = PaintingStyle.stroke..strokeWidth = 2);

    // Whiskers
    final whiskerPaint = Paint()..color = Colors.black87..style = PaintingStyle.stroke..strokeWidth = 1.5;
    canvas.drawLine(Offset(cx - 28, cy - 4), Offset(cx - 45, cy - 8), whiskerPaint);
    canvas.drawLine(Offset(cx - 28, cy + 2), Offset(cx - 45, cy + 2), whiskerPaint);
    canvas.drawLine(Offset(cx + 28, cy - 4), Offset(cx + 45, cy - 8), whiskerPaint);
    canvas.drawLine(Offset(cx + 28, cy + 2), Offset(cx + 45, cy + 2), whiskerPaint);
  }
  @override bool shouldRepaint(covariant CustomPainter oldDelegate) => false;
...
with open('lib/screens/games/cat_ball_throw_game.dart', 'w') as f:
    f.write(code)
