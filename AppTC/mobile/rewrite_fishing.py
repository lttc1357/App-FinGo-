...
class _FishingCatGameState extends State<FishingCatGame> with TickerProviderStateMixin {
  ...
  void startGame() {
    ...
    gameTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if(timeRemaining > 0) setState(() => timeRemaining--);
      else gameOver();
    });
  ...
  void generateEntities() {
    entities.clear();
    for(int i=0; i<5; i++) entities.add(Entity(x: random.nextDouble() * 0.8 + 0.1, y: random.nextDouble() * 0.2 + 0.35, radius: 0.04, weight: 1, score: 10, type: 'fish', color: const Color(0xFFFF9800), direction: random.nextBool() ? 1:-1));
    for(int i=0; i<3; i++) entities.add(Entity(x: random.nextDouble() * 0.8 + 0.1, y: random.nextDouble() * 0.2 + 0.55, radius: 0.035, weight: 2, score: 30, type: 'rare_fish', color: const Color(0xFFE040FB), direction: random.nextBool() ? 1:-1));
    for(int i=0; i<2; i++) entities.add(Entity(x: random.nextDouble() * 0.8 + 0.1, y: random.nextDouble() * 0.2 + 0.45, radius: 0.045, weight: 2, score: 40, type: 'jellyfish', color: Colors.pinkAccent, direction: random.nextBool() ? 1:-1));
    for(int i=0; i<3; i++) entities.add(Entity(x: random.nextDouble() * 0.8 + 0.1, y: random.nextDouble() * 0.2 + 0.7, radius: 0.06, weight: 5, score: -10, type: 'rock', color: Colors.grey.shade800, direction: 0));
          ...
          child: Column(
            ...
            children: [
               ...
               Container(
                 ...
                 decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: Colors.blue.withOpacity(0.2), blurRadius: 15)]),
                 child: Column(
                   children: [
          ...
          child: Container(
            ...
            child: Stack(
               children: [
                  ...
                  Positioned(top: 80, left: 40, child: Icon(Icons.cloud, color: Colors.white.withOpacity(0.7), size: 70)),
                  Positioned(top: 60, right: 120, child: Icon(Icons.cloud, color: Colors.white.withOpacity(0.5), size: 50)),
                  
                  Positioned(
                    top: MediaQuery.of(context).size.height * 0.22, left: 0, right: 0,
                    child: Container(height: 5, color: Colors.lightBlue.shade100.withOpacity(0.5))
                  ),
                      ...
                      child: Row(
                        ...
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(color: Colors.white.withOpacity(0.9), borderRadius: BorderRadius.circular(16), boxShadow: const [BoxShadow(color: Colors.black26, blurRadius: 4)]),
                  ...
                  if(!isPlaying && !isGameOver)
                    Center(
                      child: ElevatedButton(
                        ...
                        onPressed: startGame,
                        child: Text('BẮT ĐẦU CÂU!', style: GoogleFonts.nunito(fontSize: 24, fontWeight: FontWeight.black, color: Colors.white)),
                      )
                    )
               ],
            )
...
class FishingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for(var b in bubbles) canvas.drawCircle(Offset(b.x * size.width, b.y * size.height), b.size, Paint()..color = Colors.white.withOpacity(0.4)..style = PaintingStyle.fill);
  ...
  void _drawEntity(Canvas canvas, Entity e, Offset pos, double size) {
    ...
    if(e.type == 'fish' || e.type == 'rare_fish') {
       ...
       canvas.drawPath(tail, Paint()..color = e.color.withOpacity(0.9));
       ...
       canvas.drawArc(Rect.fromCenter(center: Offset(0, -size*0.2), width: size*2, height: size*2), pi, pi, true, Paint()..color = e.color.withOpacity(0.8));
       for(int i=0; i<4; i++) {
         canvas.drawLine(Offset(-size*0.6 + i*(size*0.4), 0), Offset(-size*0.6 + i*(size*0.4) + sin(e.x*50)*size*0.3, size*1.2), Paint()..color=e.color.withOpacity(0.8)..strokeWidth=2);
       }
